    ...
    n.prototype.refresh = function() {
        ...
        t = {
            ...
            for (var s in this.waypoints[n]) {
                ...
                p.element !== p.element.window && (f = p.adapter.offset()[r.offsetProp]),
                "function" == typeof c ? c = c.apply(p) : "string" == typeof c && (c = parseFloat(c),
                -1 < p.options.offset.indexOf("%") && (c = Math.ceil(r.contextDimension * c / 100))),
                a = r.contextScroll - r.contextOffset,
                p.triggerPoint = f + a - c,
                l = d < r.oldScroll,
                u = p.triggerPoint >= r.oldScroll,
                h = !l && !u,
                !w && (l && u) ? (p.queueTrigger(r.backward),
                o[p.group.id] = p.group) : !w && h ? (p.queueTrigger(r.forward),
                o[p.group.id] = p.group) : w && r.oldScroll >= p.triggerPoint && (p.queueTrigger(r.forward),
    ...
    $.fn.counterUp = function(options) {
      ...
      return this.each(function() {
        ...
        $this.waypoint(function() {
          var numbersArray = [],
              divisions = counterSettings.time / counterSettings.delay,
              originalText = $this.text(),
              isCommaSeparated = /[0-9]+,[0-9]+/.test(originalText);
          
          // Remove commas from the original number
          originalText = originalText.replace(/,/g, "");
  
          // Check if the number is a float
          var isFloat = /^[0-9]+\.[0-9]+$/.test(originalText),
              decimalPlaces = isFloat ? (originalText.split(".")[1] || []).length : 0;
  
          // Generate number steps
          for (var i = divisions; i >= 1; i--) {
            var stepValue = parseInt(originalText / divisions * i, 10);
  
            if (isFloat) {
              stepValue = parseFloat(originalText / divisions * i).toFixed(decimalPlaces);
            }
  
            if (isCommaSeparated) {
              // Add commas back to the step value
              while (/(\d+)(\d{3})/.test(stepValue.toString())) {
                stepValue = stepValue.toString().replace(/(\d+)(\d{3})/, '$1,$2');
              }
            }
  
            numbersArray.unshift(stepValue);
          }
  
          // Store the array in data
          $this.data("counterup-nums", numbersArray);
          $this.text("0"); // Start with 0
  
          // Recursive function to update the text
          var updateCounter = function() {
            var nums = $this.data("counterup-nums");
  
            // Ensure the array is still valid before using shift()
            if (Array.isArray(nums) && nums.length > 0) {
              $this.text(nums.shift());
              
              if (nums.length) {
                setTimeout(updateCounter, counterSettings.delay);
              } else {
          ...
          // Store the function and trigger it
